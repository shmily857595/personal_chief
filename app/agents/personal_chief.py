...
# 4.添加会话记忆管理
# 连接sqlite
connection = sqlite3.connect("../db/person_chief.db", check_same_thread=False)
# 初始化checkpoint
...
async def get_messages(thread_id: str):
    ...
    channel_vaules = checkpoint.get("channel_vaules")
    if not channel_vaules:
        return []

    messages = channel_vaules.get("messages", [])
    if not messages:
        return []
